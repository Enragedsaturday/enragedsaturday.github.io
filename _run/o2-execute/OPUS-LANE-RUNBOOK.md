...
**OPUS SESSION-LIMIT (wave-9, 2026-07-11 ~08:53):** the Claude ACCOUNT has its own usage/session limit (separate from the codex window). 9 waves × 10 parallel reviewers exhausted it — reviewer w9-03 failed "You've hit your session limit". This is the OPUS analog of the codex burn. LESSON: run the Opus lane at REDUCED parallelism (≈4-5 reviewers/wave, not 10) and pace across quota windows. The orchestrator (Fable/Opus) shares this account quota, so over-parallel reviewer fleets starve the main loop too. On resume: lower to ~5/wave; w9-03's 4 groups are auto-re-picked (they show opus-pending in the checkpoint). Codex 2-lens is DONE (1357/1357) so ALL groups are now Opus-eligible.
