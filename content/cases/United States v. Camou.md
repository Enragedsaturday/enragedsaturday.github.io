...
**Status: Unverified — subsequent treatment not yet machine-verified.** This page was authored from a CourtListener-verified identity stub and renders under the ⚪ banner until machine verification is complete. *Camou* carries *[[Riley v. California|Riley]]* into the automobile-exception setting: a cell phone found in a car is not a "container" that probable cause to search the vehicle allows officers to open, so a warrant is required to search the phone's data even where the vehicle exception otherwise applies.
