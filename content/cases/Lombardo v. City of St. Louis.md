...
**Status: Unverified — subsequent treatment not yet machine-verified.** This page was authored from a CourtListener-verified identity stub; its citator and progeny history have not completed the two-key verification, so it renders under the ⚪ banner until machine verification is complete. *Lombardo* is a pretrial-detainee (Fourteenth Amendment) excessive-force decision governed by *[[Kingsley v. Hendrickson|Kingsley]]*'s objective-reasonableness standard, the detention analog to *[[Graham v. Connor]]*; it reaffirms that prone-restraint reasonableness is fact-specific, not categorical.
